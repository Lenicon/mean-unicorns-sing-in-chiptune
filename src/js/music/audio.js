...
let getSectionSteps = (sec) => sec ? Math.max(1, Math.round(sec.dur / ((60 / 130) * 4))) * 16 : 128;
let ac;
...
let bus_ = () => { ensureBus_(); return shaperNode; };
let busClean_ = () => { ensureBus_(); return cleanGain; };
let master_ = () => { ensureBus_(); return masterGain; };
...
let F = (n) => n == null ? 0 : 440 * Math.pow(2, n / 12);
let safeTime = (t, context) => (typeof t === 'number' && isFinite(t)) ? t : context.currentTime;
function sfxPew(t) {
    const c = ac_();
